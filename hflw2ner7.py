...
def train():
    ...
    dataloader_train = DataLoader(dataset_train,
                                  batch_size=batch_size * accum_step,
                                  shuffle=True,
    ...
    oneepoch_training_step = oneepoch_stepping_steps * accum_step
    ...
    for epoch in range(num_epochs):
        ...
        for batch, (sen, ls, span) in enumerate(dataloader_train):
            ...
            seqlen2sum = torch.sum(torch.square(ls), dtype=torch.float)

            for k in range(accum_step):
                minils = ls[k * batch_size:(k + 1) * batch_size]
                minimaxls = minils.max()

                minisen["input_ids"] = sen["input_ids"][k * batch_size:(k + 1) * batch_size, :minimaxls + 2]
                minisen["attention_mask"] = sen["attention_mask"][k * batch_size:(k + 1) * batch_size, :minimaxls + 2]

                minispan = span[k * batch_size:(k + 1) * batch_size, :minimaxls, :minimaxls]

                _, loss, _, _, _ = model(minisen, minils, minispan)

                miniseqlen2sum = torch.sum(torch.square(minils), dtype=torch.float)

                losst = loss / miniseqlen2sum
                total_loss += losst.item()

                loss = loss / seqlen2sum

                loss.backward()

                print("\repoch: %d  %d|%d  loss: %f " % (
                    epoch + 1, batch * accum_step + k + 1, oneepoch_training_step, losst.item()), end="")
