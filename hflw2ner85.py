...
mycheckpoint = "models/hflw2ner85"
...
class MYW2NER(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask, seqlen, labels=None):
        if self.training:
            ...
            embeds = embeds + torch.zeros_like(embeds).uniform_(-mag_norm, mag_norm)

            out = self.bert(None, attention_mask, inputs_embeds=embeds)
        else:
...
def train():
    ...
    logger.info(
        "基于QLora微调BERT实现医疗NER任务，超参设置 --mode train --num_epochs %d --lr %e --batch_size %d --accum_steps %d --model_path %s --pretrained_checkpoint %s" % (
