...
def tri(x, y, N, hidden):
    ...
    aa = torch.tile(torch.unsqueeze(a, dim=2), [1, 1, hidden])

    bb = aa * y[0]

    aaa = torch.tile(torch.unsqueeze(torch.logical_not(a), dim=2), [1, 1, hidden])
...
class MYW2NER(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.tri = Parameter(init.xavier_normal_(torch.empty(2, self.config.hidden_size)))
    ...
    def forward(self, bertinputs, seqlen, span=None):
        ...
        xx = tri(xx, self.tri, N, self.config.hidden_size)
